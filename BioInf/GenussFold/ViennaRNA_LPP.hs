...
module BioInf.GenussFold.ViennaRNA_LPP where
...
import           BioInf.GenussFold.Grammars.ViennaRNA_LPP
...
energyMinAlg input penalty compound = SigEnergyMin
  ...
  , kpkn = \ () () a () b c       -- :: A C A B C B
    -> let m = maximum [a,b,c] in if m < 0 then a + b + c + penalty else ignore
  , mpkn = \ () () () a b c       -- :: A B C A B C
    -> let m = maximum [a,b,c] in if m < 0 then a + b + c + penalty else ignore
  , lpkn = \ () () () a () b c d  -- :: A B C A D B C D
...
pretty = SigEnergyMin
  ...
  , hpkn = \ () () [a1,a2] [b1,b2]
    -> [a1 ++ b1 ++ a2 ++ b2]
  , kpkn = \ () () [a1,a2] () [c1,c2] [b1,b2]              -- :: A C A B C B
    -> [a1 ++ c1 ++ a2 ++ b1 ++ c2 ++ b2]
  , mpkn = \ () () () [a1,a2] [b1,b2] [c1,c2]              -- :: A B C A B C
    -> [a1 ++ b1 ++ c1 ++ a2 ++ b2 ++ c2]
  , lpkn = \ () () () [a1,a2] () [b1,b2] [c1,c2] [d1,d2]   -- :: A B C A D B C D
...
--                                                                        1  2  3  4  5  6  7  8  9  1  2  3  4
runInsideForward :: BS.ByteString -> VU.Vector Char -> Int -> Ptr() -> Z:.X:.X:.X:.X:.X:.X:.X:.X:.X:.T:.T:.T:.T
runInsideForward i iv p c = mutateTablesWithHints (Proxy :: Proxy MonotoneMCFG)
                   $ gEnergyMin (energyMinAlg i p c)
                        ...
                        (ITbl 0 0 EmptyOk (PA.fromAssocs (subword 0 0) (subword 0 n) (466999) [])) -- 5
                        (ITbl 0 0 EmptyOk (PA.fromAssocs (subword 0 0) (subword 0 n) (466999) [])) -- 6
                        (ITbl 0 0 EmptyOk (PA.fromAssocs (subword 0 0) (subword 0 n) (466999) [])) -- 7
                        (ITbl 0 0 EmptyOk (PA.fromAssocs (subword 0 0) (subword 0 n) (466999) [])) -- 8
                        (ITbl 0 0 EmptyOk (PA.fromAssocs (subword 0 0) (subword 0 n) (466999) [])) -- 9
...
--                                                                          1  2  3  4  5  6  7  8  9  1  2  3  4
runInsideBacktrack :: BS.ByteString -> VU.Vector Char -> Int -> Ptr() -> Z:.X:.X:.X:.X:.X:.X:.X:.X:.X:.T:.T:.T:.T -> [[String]] -- for the non-terminals:.T:.T
--                              1  2  3  4  5  6  7  8  9  1  2  3  4
runInsideBacktrack i iv p c (Z:.a:.b:.d:.e:.f:.j:.k:.l:.s:.t:.u:.v:.w) = unId $ axiom g -- Axiom from the Start Nonterminal S -> a_Struct-
--           1  2  3  4  5  6  7  8  9  1  2  3  4
  where !(Z:.g:._:._:._:._:._:._:._:._:._:._:._:._) = gEnergyMin (energyMinAlg i p c<|| pretty)
  -- where !(Z:.g:.h:.l:.m:.n:.o:.p) = gEnergyMin (energyMinAlg i <|| prettyPaths i)
                          ...
                          (toBacktrack d (undefined :: Id y -> Id y)) -- 3
                          (toBacktrack e (undefined :: Id y -> Id y)) -- 4
                          (toBacktrack f (undefined :: Id y -> Id y)) -- 5
                          (toBacktrack j (undefined :: Id y -> Id y)) -- 1
                          (toBacktrack k (undefined :: Id y -> Id y)) -- 2
                          (toBacktrack l (undefined :: Id y -> Id y)) -- 3
                          (toBacktrack s (undefined :: Id y -> Id y)) -- 4
                          (toBacktrack t (undefined :: Id y -> Id y)) -- 4
                          (toBacktrack u (undefined :: Id y -> Id y)) -- 4
                          (toBacktrack v (undefined :: Id y -> Id y)) -- 4
                          (toBacktrack w (undefined :: Id y -> Id y)) -- 4
                          (ntPos iv)
                          (idxStrng1 1 31 iv)
