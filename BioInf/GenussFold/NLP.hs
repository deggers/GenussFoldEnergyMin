...
bpmax p = SigPKN
  ...
  , pkn = \ s () () x y -> let m = minimum [x,y] in if m >= 1 then x + y + p + s else -888888  -- iff one is zero than penalty 6 laufveraiblen _> n8
  , nil = \ ()      -> 0
...
--  khp <<< X  c  Y    X    c  S  c Z    Y    c      Z
  , pkn = \ [s] () () [x1,x2] [y1,y2] -> [s ++ x1 ++ y1 ++ x2 ++ y2]
  , nil = \ ()      -> [""]
...
runInsideForward :: VU.Vector Char -> Int -> Z:.X:.X:.T:.T
...
runInsideBacktrack :: VU.Vector Char -> Int -> Z:.X:.X:.T:.T -> [[String]]
