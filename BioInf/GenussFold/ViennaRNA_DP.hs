...
module BioInf.GenussFold.ViennaRNA_DP where
...
import           BioInf.GenussFold.Grammars.ViennaRNA_DP
...
pretty = SigEnergyMin
  ...
  , hpk = \ () () [x1,x2] [y1,y2]
    -> [x1 ++ y1 ++ x2 ++ y2]
  , pk1 = \ (Z:.():.(lPos,jPos)) (Z:.():.[s2]) [x1,x2] (Z:.(iPos,kPos):.()) (Z:.[s1]:.())
    -> [ x1 ++ replicate (kPos-iPos-1) '.' ++ "{" ++ s1 , "}" ++ replicate (jPos-lPos-1) '.' ++ s2++ x2]
  , pk2 = \ (Z:.():.(lPos,jPos)) (Z:.():.[s2]) [x1,x2] (Z:.(iPos,kPos):.()) (Z:.[s1]:.())
    -> [ x1 ++ replicate (kPos-iPos-1) '.' ++ "[" ++ s1 , "]" ++ replicate (jPos-lPos-1) '.' ++ s2 ++ x2]
