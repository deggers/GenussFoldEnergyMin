...
module BioInf.GenussFold.ViennaRNA_RG where
...
import           BioInf.GenussFold.Grammars.ViennaRNA_RG
...
energyMinAlg input penalty compound = SigEnergyMin
  { nil  = \ () -> 0
  , khp = \ () () x y -- ::  H-Pseudoknot -  Only if poth parts are contributing a negative energy
    -> let m = maximum [x,y] in if m < 0 then x + y + penalty else ignore
   -- interiorLoops instead of stacking
  , pk1 = \ (Z:.():.(lPos,jPos)) y (Z:.(subtract 1 -> iPos, subtract 1 -> kPos):.()) (Z:.():.m) (Z:.n:.()) -> if
      ...
      | otherwise -> ignore
  , pk2 = \ (Z:.():.(lPos,jPos)) y (Z:.(subtract 1 -> iPos, subtract 1 -> kPos):.()) (Z:.():.m) (Z:.n:.()) -> if
      | (minimum [lPos,jPos,iPos,kPos] >= 0)
        && (jPos < BS.length input)
        && pairs (BS.index input iPos) (BS.index input jPos)
        && pairs (BS.index input kPos) (BS.index input lPos)
...
pretty = SigEnergyMin
  ...
  , khp = \ () () [x1,x2] [y1,y2]
    -> [x1 ++ y1 ++ x2 ++ y2]
  , pk1 = \ (Z:.():.(lPos,jPos)) [x1,x2] (Z:.(iPos,kPos):.()) (Z:.():.[s2]) (Z:.[s1]:.())
    -> [ x1 ++ replicate (kPos-iPos-1) '.' ++ "{" ++ s1 , "}" ++ replicate (jPos-lPos-1) '.' ++ x2 ++ s2]
  , pk2 = \ (Z:.():.(lPos,jPos)) [x1,x2] (Z:.(iPos,kPos):.()) (Z:.():.[s2]) (Z:.[s1]:.())
    -> [ x1 ++ replicate (kPos-iPos-1) '.' ++ "[" ++ s1 , "]" ++ replicate (jPos-lPos-1) '.' ++ x2 ++ s2]
...
energyMin (NumBT k) (PenPK p) inp = unsafePerformIO $ do
  ...
  let
    ...
    !(Z:.a:.b:.e:.f:.g:.j) = runInsideForward i iv p c
    z = unId $ axiom a -- gets the value from the table
    bs = take k $ runInsideBacktrack i iv p c (Z:.a:.b:.e:.f:.g:.j)
...
runInsideForward :: BS.ByteString -> VU.Vector Char -> Int -> Ptr() -> Z:.X:.X:.X:.X:.T:.T
...
runInsideBacktrack :: BS.ByteString -> VU.Vector Char -> Int -> Ptr() -> Z:.X:.X:.X:.X:.T:.T -> [[String]] -- for the non-terminals
runInsideBacktrack i iv p c (Z:.a:.b:.e:.f:.j:.k) = unId $ axiom g -- Axiom from the Start Nonterminal S -> a_Struct-
  where !(Z:.g:._:._:._:._:._) = gEnergyMin (energyMinAlg i p c<|| pretty)
